...
// Root volunteer shell. Wires locationUtils ↔ LocationSection ↔
// useProximityAlerts ↔ ProximityToastStack so every subsystem shares
// one source of truth for the volunteer's position.
...
import { LocationSection, loadSavedLocation, type SavedLocation } from './LocationSection';
import { ProximityToastStack } from './ProximityToast';
import { useProximityAlerts, type NearbyTask } from './useProximityAlerts';
import {
  getDistanceKm,
  formatDistance,
  type GeoCoords,
...
interface VolunteerTask {
  ...
  skill: string;
  distance: string;        // pre-formatted label, updated when coords known
  distanceKm?: number;     // raw km, used for scoring
...
const DUMMY_TASKS: VolunteerTask[] = [
  {
    ...
    // Anna Nagar, Chennai approx coords
    coords: { lat: 13.0878, lng: 80.2101 },
  ...
  },
  {
    ...
    coords: { lat: 13.0368, lng: 80.2676 },
...
const priorityToUrgency = (p: string): VolunteerTask['urgency'] => {
  ...
  if (l === 'emergency' || l === 'critical') return 'Critical';
...
const firestoreStatusToLocal = (s: string): TaskStatus => {
  ...
  if (l === 'in progress' || l === 'active' || l === 'assigned') return 'active';
  return 'new';
};
...
const mapFirestoreTask = (docId: string, data: Record<string, unknown>): VolunteerTask => {
  ...
  // Firestore tasks may store coords as {lat, lng} or GeoPoint
  const raw = data.coords as { lat?: number; lng?: number } | undefined;
...
/**
 * Re-computes distance labels & km for all tasks given the volunteer's position.
 * Returns a new array (pure).
 */
const applyDistances = (tasks: VolunteerTask[], vol: GeoCoords): VolunteerTask[] =>
  tasks.map(t => {
    if (!t.coords) return t;
    const km = getDistanceKm(vol, t.coords);
    return {
      ...t,
      distanceKm: km,
      distance: formatDistance(km),
      whyChosen: { ...t.whyChosen, distance: formatDistance(km) },
    };
  });
...
const fetchAllocationReasoning = async (task: VolunteerTask): Promise<AllocationResult> => {
  const distCtx = task.distanceKm != null
    ? `- Actual distance: ${formatDistance(task.distanceKm)}`
    : '';
...
const TaskCard = ({
  ...
  const handleExpand = async () => {
    ...
    if (next && !aiResult && !aiLoading) {
      ...
      try   { setAiResult(await fetchAllocationReasoning(task)); }
  ...
  return (
    <motion.div layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: task.status === 'completed' ? 0.55 : 1, y: 0 }} transition={{ duration: 0.25 }}>
      <Card className="overflow-hidden">
        <div className="p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px] text-brand-text-secondary">
            <div
              className="flex items-center gap-1.5 cursor-pointer hover:text-brand-primary hover:underline underline-offset-2 transition-colors"
              onClick={() => {
                const url = task.coords
                  ? `https://www.google.com/maps?q=${task.coords.lat},${task.coords.lng}`
                  : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(task.location)}`;
                window.open(url, '_blank');
              }}
            >
              <MapPin className="w-3 h-3 text-brand-primary shrink-0" />{task.location}
            </div>
            <div className="flex items-center gap-1.5">
              {isFirestore ? <Calendar className="w-3 h-3 text-brand-primary shrink-0" /> : <Clock className="w-3 h-3 text-brand-primary shrink-0" />}
              {task.date}
            </div>
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                {aiLoading && (
                  <div className="flex items-center gap-2 py-3 text-[11px] text-brand-text-secondary">
                    <Loader2 className="w-3.5 h-3.5 animate-spin text-brand-primary" />Analysing volunteer profile…
                  </div>
                )}
                {aiError && <p className="text-[11px] text-red-500 py-2">Failed to load AI analysis. Check your Groq key.</p>}
                {aiResult && (
          ...
          <div className="flex gap-2 pt-1">
            ...
            {task.status === 'active' && (
              <Button className="flex-1 text-[10px] h-8 bg-green-600 hover:bg-green-700 font-bold" onClick={() => onComplete(task.id, task.firestoreId)}>
...
const SettingsPage = ({
  ...
  return (
    <div className="space-y-4">
      {/* ── Geolocation card — wired with onLocationChange callback ── */}
      <LocationSection onLocationChange={onLocationChange} />
...
export const VolunteerDashboard = () => {
  ...
  // ── Volunteer's live coords (single source of truth) ──────────────────────
  const [volunteerCoords, setVolunteerCoords] = useState<GeoCoords | null>(() => {
  ...
  // ── When volunteer coords change, re-compute distances on all tasks ────────
  useEffect(() => {
    if (!volunteerCoords) return;
    setTasks(prev => sortTasks(applyDistances(prev, volunteerCoords)));
  }, [volunteerCoords]);

  // ── Build NearbyTask list for the proximity hook ───────────────────────────
  const nearbyTasksForHook = useMemo<NearbyTask[]>(() =>
    tasks
  ...
  const { alerts, dismiss } = useProximityAlerts(volunteerCoords, nearbyTasksForHook);
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        const merged          = sortTasks([...fetched, ...filteredDummy]);
        // Apply distances immediately if we already have coords
        setTasks(volunteerCoords ? applyDistances(merged, volunteerCoords) : merged);
      } catch (err) {
  ...
  /** Called by LocationSection when a new GPS fix is captured. */
  const handleLocationChange = (coords: GeoCoords) => {
    setVolunteerCoords(coords);
    // Distance re-computation is handled by the useEffect above.
  };
  ...
  const PageMap: Record<NavKey, ReactElement> = {
    ...
    settings:     <SettingsPage    displayName={displayName} onLocationChange={handleLocationChange} />,
  };
  ...
  return (
    <div className="flex h-screen bg-brand-background overflow-hidden">
      ...
      {/* ── Proximity alert toasts — rendered outside the scrollable content ── */}
      <ProximityToastStack alerts={alerts} onDismiss={dismiss} bottomOffset={88} />
    </div>
  );
};